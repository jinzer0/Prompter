...
이 문서는 Hyper-Waterfall 방법론 자체의 GitHub Release/tag 기준, upstream release PR, 기존 적용 저장소 update protocol을 정의한다. 일반 task 브랜치 운용은 `git_workflow_guide.md`를 따른다.

Prompter 저장소 안에서 이 문서의 release PR 절차는 Hyper-Waterfall 프레임워크 upstream에만 적용한다. Prompter 제품 release 절차는 별도 승인된 task에서 정하기 전까지 이 문서가 지시하지 않는다.

## Canonical 배포 기준

Hyper-Waterfall 방법론 자체의 canonical 배포 단위는 GitHub Release/tag다. `master`에 반영된 상태를 tag로 고정하고, 해당 release의 `templates/manifest.json`과 `docs/migrations/`를 기준으로 기존 적용 저장소가 업데이트한다.
...
일반 task PR과 framework release PR은 분리한다. 기존 적용 저장소 업데이트는 upstream release 이후 별도 Hyper-Waterfall 버전 업데이트 PR로 수행한다.

## Framework Release PR 흐름

이 절차는 Hyper-Waterfall 프레임워크 upstream에서만 실행한다. Prompter에는 별도 release 브랜치가 없으므로 `master`에서 `master`로 PR을 만들지 않는다.

```bash
# upstream 프레임워크 저장소의 승인된 release 절차를 따른다.
```
...
실제 tag 생성과 GitHub Release 발행은 upstream 프레임워크 저장소의 별도 승인된 release 단계에서 수행한다. Prompter 제품 release는 별도 승인된 Prompter task 없이는 수행하지 않는다.
...
Hyper-Waterfall 버전 업데이트 PR 브랜치를 별도 prefix로 만들지 않는 이유는 작업 추적 기준을 GitHub Issue와 하이퍼-워터폴 산출물로 유지하기 위해서다. CLI나 자동화가 PR 후보를 만들더라도 먼저 판단 결과를 출력하고, 승인된 이슈 번호를 받은 뒤 `local/task{N}` -> `publish/task{N}` -> `master` 규칙을 따른다.
