...
- **`master`**: 모든 작업 PR이 모이는 개발 통합 브랜치. 새 작업 브랜치는 최신 `origin/master` 기준으로 만든다.
- **`local/taskN`**: 이슈 번호 N의 로컬 작업 브랜치. 단계 커밋과 보고서 커밋은 이 브랜치에 쌓는다.
- **`publish/taskN`**: `local/taskN`을 원격에 게시하기 위한 PR용 브랜치. PR merge 후 삭제한다.
- **Open PR**: 검토 가능한 상태의 PR. 하이퍼-워터폴 최종 보고 후 `master` 대상으로 만든다.
...
| 브랜치 | 용도 |
|--------|------|
| `master` | 개발 통합. 작업 PR이 merge되는 기본 브랜치 |
| `local/task{num}` | 타스크별 작업 |
| `publish/task{num}` | `master` 대상 PR 생성을 위한 원격 게시 브랜치. PR merge 후 삭제 |
...
local/task{N} ── 커밋 · 커밋 · 커밋 ──→ publish/task{N} push
                                          │
                                          └─→ master 대상 PR → 리뷰 → merge
                                                                       │
                                                                       └─→ master 누적
                                                                              │
                                                                                └─→ upstream framework release는 별도 저장소 절차에서 수행
```
...
- **`master` 대상 PR**: 작업 단위 PR은 기본적으로 Open PR로 생성하고, 최종 보고와 검증 결과를 PR 본문에 반영한 상태에서 review/merge 한다.
- **merge 전략**: `master` 대상 PR은 merge commit 유지 또는 `--no-ff` 원칙을 기본으로 한다. squash merge는 단계별 커밋 의미가 사라질 수 있으므로 기본값으로 두지 않는다.
- **framework release**: Hyper-Waterfall 프레임워크 release 발행은 upstream 전용 절차다. Prompter에서는 `master`에서 `master`로 PR을 만들지 않고, Prompter 제품 release 절차는 별도 승인된 task에서 정한다.

## PR 유형 구분

일반 task PR과 framework release PR은 분리한다. task PR은 `local/taskN -> publish/taskN -> master` 흐름을 따른다. framework release PR은 Hyper-Waterfall upstream 전용이며 Prompter에서 `master -> master` PR로 만들지 않는다. 기존 적용 저장소 업데이트는 upstream release 이후 별도 Hyper-Waterfall 버전 업데이트 PR로 수행한다. Release/tag와 update protocol 상세는 [`release_update_protocol.md`](release_update_protocol.md)를 따른다.

| 유형 | 목적 | 브랜치 흐름 | PR 제목 |
|---|---|---|---|
| task PR | 저장소 기능, 문서, 운영 작업을 이슈 단위로 반영 | `local/task{N}` -> `publish/task{N}` -> `master` | `Task #{N}: {작업 제목}` |
| framework release PR | upstream 프레임워크 저장소에서 누적된 프레임워크 변경을 release 대상으로 승격하고 tag 기준을 만든다 | upstream release 흐름 | `Release: {version}` |
| Hyper-Waterfall 버전 업데이트 PR | 기존 적용 저장소를 현재 version에서 목표 release/tag로 올린다 | `local/task{N}` -> `publish/task{N}` -> `master` | `Task #{N}: Hyper-Waterfall {fromVersion} -> {toVersion} 버전 업데이트` |

## 메인테이너 워크플로우

```bash
# 1. local/taskN → publish/taskN push + master 대상 Open PR
git checkout local/task17
git push origin local/task17:publish/task17
gh pr create --base master --head publish/task17 --title "Task #17: 제목" --body-file /tmp/task17-pr-body.md

# 2. master 대상 PR 리뷰 + merge
gh pr review --approve
gh pr merge --merge --delete-branch

# 3. Hyper-Waterfall framework release는 upstream 절차에서만 수행

# 4. framework release tag 생성 전 manifest/migration 확인
...
# 2. Fork한 저장소에서 작업
git clone https://github.com/{contributor}/Prompter.git
git checkout -b feature/my-task
# ... 작업 + 커밋 ...
git push origin feature/my-task

# 3. 원본 저장소의 master로 PR 생성
gh pr create --repo jinzer0/Prompter --base master --head {contributor}:feature/my-task --title "제목"
...
### `master`에 rebase가 필요해 보일 때

기본 흐름은 `master`을 `git pull --ff-only`로 최신화하고, 새 `local/taskN` 브랜치를 최신 `origin/master`에서 만드는 것이다. 진행 중인 작업 브랜치를 임의로 rebase하지 않는다. PR 충돌이나 오래된 기준 브랜치 문제가 생기면 먼저 `git fetch origin`과 충돌 파일 목록을 확인하고, rebase/merge 중 어떤 방식으로 회복할지 작업지시자 승인 후 진행한다.
...
PR이 `MERGED` 상태인지 먼저 확인한다. merge 확인 후 `master`로 돌아와 최신화하고, 원격 `publish/taskN`과 로컬 `local/taskN`을 정리한다. 이 절차는 [`pr-merge-cleanup`](../skills/pr-merge-cleanup/SKILL.md) SKILL이 문서화한 순서를 따른다.
