...
const generatedStorageRemnantFileName = new RegExp(
  `^(?:\\.(?:notarization-resume|notarization-final)\\.json\\.${uuid}\\.tmp|\\.notary-${uuid}\\.json\\.${uuid}\\.tmp|\\.\\.notarization-submit\\.reclaim\\.${uuid}\\.tmp|\\.notarization-submit\\.(?:claim|reclaim)\\.${uuid}\\.remove)$`,
  "u",
)
